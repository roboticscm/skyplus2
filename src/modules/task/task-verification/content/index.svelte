<script lang="ts">
  ...
  // Other vars
  let selectedData: TaskVerification;
  let saveOrUpdateSub;
  /**
  ...
  const saveUpdateUri = 'task/task-verification/save-or-update';
  ...
  const doSaveOrUpdate = (ob$: Observable<any>) => {
    saveOrUpdateSub = ob$
      ...
      .subscribe({
        ...
        error: (error) => {
          Debug.errorSection('Task Verification - doSaveOrUpdate', error);
          saveRunning$.next(false);
        },
      });
  };
...
<section class="view-content-main">
  <form class="form" on:keydown={(event) => form.errors.clear(event.target.name)}>
    <div class="row ">
      ...
      <div class="col-xs-24 col-sm-6">
        <FloatTextInput
          placeholder={T('COMMON.LABEL.PERCENTAGE')}
          name="percent"
          disabled={$isReadOnlyMode$}
          bind:value={form.percent} />
        <Error {form} field="percent" />
      </div>
      <!-- // Name -->

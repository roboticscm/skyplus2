<script lang="ts">
  ...
  import { TaskQualification } from '../../types';
  ...
  // Other vars
  let selectedData: TaskQualification;
  let saveOrUpdateSub;
  /**
  ...
  const saveUpdateUri = 'task/task-qualification/save-or-update';
  ...
  const doSaveOrUpdate = (ob$: Observable<any>) => {
    saveOrUpdateSub = ob$
      ...
      .subscribe({
        ...
        error: (error) => {
          Debug.errorSection('TaskQualification - doSaveOrUpdate', error);
          saveRunning$.next(false);
        },
      });
  };

...
<section id={workListContainerId} class="view-left-main">
  <DatePicker />
  <Filter
    on:itemChange={onItemChangeFilter1}
    list={list.filter((it) => [filter2Id, filter3Id].indexOf(it.id.toString()) < 0)}
    name="filter1"
    bind:value={filter1Text} />
  <Filter
    on:itemChange={onItemChangeFilter2}
